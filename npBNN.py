...
# load tensorflow object
load_tensorflow_object = False
if load_tensorflow_object:
    from np_bnn import BNN_env, BNN_files, BNN_lib
    ...
    # load data (2 files: features and labels)
    f = "/Users/xhofmt/GitHub/madagascar_iucnn/data/features.txt"
    l = "/Users/xhofmt/GitHub/madagascar_iucnn/data/labels.txt"
    # with testsize=0.1, 10% of the data are randomly selected as test set
    # if all_class_in_testset = 1: 10% of the samples and a minimum of 1 sample
    # for each class are represented in the test set
    dat = BNN_files.get_data(f,l,
    ...
    output = BNN_lib.RunPredict(dat['data'],init_weights,actFun=BNN_lib.genReLU())
    pred_labels = np.argmax(output,axis=1)
    true_labels = dat['labels']
    BNN_lib.CalcAccuracy(output,true_labels)
