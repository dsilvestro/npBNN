...
def get_pdp(data,
    ...
    if estimation_mode == 'classification':
        for n in range(num_pdp_steps):
            ...
            softmax_probs = np.zeros((len(weights), data.shape[0], size_output))
            for i in range(len(weights)):
                actFun_i = actFun
                actFun_i.reset_prm(alphas[i])
                softmax_probs[i, :, :] = RunPredict(feat, weights[i],
                                                    actFun=actFun_i,
                                                    output_act_fun=output_act_fun,
                                                    data_transform=data_transform)

            cum_softmax_probs = np.cumsum(softmax_probs, axis=2)
            pdp[n, :, 0] = np.mean(cum_softmax_probs, axis=(0,1))
            # Should this be an HPD?
            probs_quantiles = np.quantile(np.mean(cum_softmax_probs, axis=0), q=(0.025, 0.975), axis=0)
...
def pdp(pickle_file, pdp_features):
    ...
    # bnn_obj._size_output
    # bnn_obj._estimation_mode
    # bnn_obj._data
    # mcmc_obj._n_post_samples
    ...
    pdp = []
    for p in range(len(pdp_features)):
        p = get_pdp(bnn_obj._data, pdp_features[p], bnn_obj._estimation_mode, bnn_obj._size_output,
                    actFun, output_act_fun, post_weights, post_alphas, post_error, data_transform)
        pdp.append(p)
